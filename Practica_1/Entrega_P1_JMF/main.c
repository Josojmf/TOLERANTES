...
void TASK_LED1 (void);
void TASK_LED2 (void);//Crea prototipo func
void TASK_LED3 (void);
void TASK_LED4 (void);
...
unsigned char txdata3[100];
unsigned char txdata4[100];
...
void main (void)
{
    ...
    OSTaskCreate(        
                ...
                (void        *)0,
                (OS_OPT       )(OS_OPT_TASK_STK_CHK|OS_OPT_TASK_STK_CLR),//chequea y otro limpia la stack
                (OS_ERR      *)&os_err);
    
    //OS TASK CREATE CON DATOS DEL 2 HILO
      OSTaskCreate(        
                (OS_TCB     *)&task_led2_TCB, //
      ...
      OSTaskCreate(        
                ...
                (OS_OPT       )(OS_OPT_TASK_STK_CHK|OS_OPT_TASK_STK_CLR),//chequea y otro limpia la stack
                (OS_ERR      *)&os_err);
    //Para crear otro hilo copia pega y cambia nombre crea stl 
    // CPU_STK task_led1_STK[1024];
    //Crea prototipo func
    //void task led 2 en el main
    //crea el funcionamiento del hilo abajo l 271
      //5 LEDS MAS con 6 prioridades
...
void TASK_LED1 (void)
{
    ...
    while(1)
    {
        sprintf(txdata,"TASK1 Green LED\r\n");
        EnviarString(txdata, Uart3);
        LED_GREEN = !LED_GREEN_Read;
        OSTimeDly(100,OS_OPT_TIME_DLY, &os_err);  //100 ticks del RTOS -> 100ms porque justo coincide la freqcuencia es 1000 MHz
    
    }
}
void TASK_LED2 (void) //Red LED
{
    OS_ERR os_err;
    
    while(1)
    {
        sprintf(txdata,"TASK2 RED LED\r\n");
        sprintf(txdata,"Tarea nueva LED Rojo\r\n");
        EnviarString(txdata, Uart3);
        LED_RED = !LED_RED_Read; //changeleds
        OSTimeDly(100,OS_OPT_TIME_DLY, &os_err);  //100 ticks del RTOS -> 100ms delay tipo eficiente
    
    }
}
void TASK_LED3 (void) //Red LED
{
    OS_ERR os_err;
    
    while(1)
    {
        sprintf(txdata,"TASK3\r\n");
        sprintf(txdata,"Tarea nueva LED Azul 1\r\n");
        EnviarString(txdata, Uart3);
        LED_BLUE1 = !LED_BLUE1_Read; //changeleds
        OSTimeDly(100,OS_OPT_TIME_DLY, &os_err);  //100 ticks del RTOS -> 100ms delay tipo eficiente
    
    }
}
void TASK_LED4 (void) //Red LED
{
    OS_ERR os_err;
    
    while(1)
    {
        sprintf(txdata,"TASK4\r\n");
        sprintf(txdata,"Tarea nueva LED Azul 2\r\n");
        EnviarString(txdata, Uart3);
        LED_BLUE2 = !LED_BLUE2; //changeleds
        OSTimeDly(100,OS_OPT_TIME_DLY, &os_err);  //100 ticks del RTOS -> 100ms delay tipo eficiente
    
    }
}
void TASK_LED5 (void) //Red LED
{
    OS_ERR os_err;
    
    while(1)
    {
        sprintf(txdata,"TASK5\r\n");
        sprintf(txdata,"Tarea nueva LED Azul 3\r\n");
        EnviarString(txdata, Uart3);
        LED_BLUE3 = !LED_BLUE3; //changeleds
        OSTimeDly(100,OS_OPT_TIME_DLY, &os_err);  //100 ticks del RTOS -> 100ms delay tipo eficiente
